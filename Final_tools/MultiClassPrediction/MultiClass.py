...
def HTML_Gen(html):
    ...
    <div class="container">
      ...
      <div class="row">
        <div class="col-sm-4">
          <img src="1.png" alt="Smiley face" height="350" width="350">
        </div>
        <div class="col-sm-4">
...
def ReturnData(TrainFile,  TestMethod, TestFile=None):
    ...
    if (TestFile == None) and (TestMethod == 'Internal'):
...
def Fit_Model(TrainData, Test_Method, Algo, Selected_Sclaer,  Workdirpath,  htmlOutDir, OutFile, htmlFname, NoOfFolds=None,  TestData=None ):
    ...
    if Test_Method == 'Internal':
        ...
        clf = OneVsRestClassifier(Algo, n_jobs=-1)

        y_score = cross_val_predict(clf, X, y, cv=int(NoOfFolds) ,method='predict_proba')
        y_pred = cross_val_predict(clf, X, y, cv=NoOfFolds )

        a =  precision_recall_fscore_support(y, y_pred, average='macro')
        ...
        print(Workdirpath, htmlOutDir, htmlFname)
        ...
        clf = OneVsRestClassifier(Algo).fit(X_train, y_train)
